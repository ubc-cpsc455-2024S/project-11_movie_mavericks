...
const MovieDetailsPopup = ({ tmdb_movie_id, onClose }) => {
  ...
  useEffect(() => {
    const fetchMovieComments = async () => {
      let movieResponse;
      try {
        movieResponse = await axios.get(`${URL}/movies/${tmdb_movie_id}`);
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const reviewResponse = await axios.post(`${URL}/reviews`, body);
      ...
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box display="flex">
            {streamingResponse && (
              <Button
                onClick={() => setStreamingDialog(true)}
                variant="outlined"
              >
                Stream now
              </Button>
              ...
              <ListItem style={{ paddingLeft: 0, paddingRight: 0 }}>
                ...
                {editReviewID === review._id ? (
                  <Rating
                    value={editRating}
                    size="small"
                    onChange={(event, rating) => setEditRating(rating)}
                  />
                ) : (
                ...
                {review.user_id === userID ? (
                  <>
                    ...
                    >
                      <DialogContent>Confirm delete comment?</DialogContent>
                      <DialogActions>
